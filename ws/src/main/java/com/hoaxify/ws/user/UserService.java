...
public class UserService {
    ...
    public void handleResetRequest(@Valid 
    PasswordResetRequest passwordResetRequest) {
        ...
        this.emailService.sendPasswordResetEmail(inDB.getEmail()
        , inDB.getPasswordResetToken());
    }


    public void updatePassword(String token, 
    @Valid PasswordUpdate passwordUpdate) {
        User inDB = userRepository.findByPasswordResetToken(token);
        if(inDB == null ){
            throw new InvalidTokenException();
        }
        inDB.setPasswordResetToken(null);
        inDB.setPassword(passwordEncoder
        .encode(passwordUpdate.password()));
        inDB.setActive(true);
        inDB.setActivationToken(null);
        userRepository.save(inDB);
    }
